...
export const Reviews = () => {
  ...
  const reviewsList = [
    ...
    },
    {
      ...
      text: "මගේ Online Cosmetics Shop එකට Premium වෙබ් සයිට් එකක් හදලා දුන්නේ ඔවුන්. මාරම Speed වගේම Customersලට ලේසියෙන් බඩු ගන්න පුළුවන් විදිහට හදලා තියෙනවා. Highly recommended!"
    },
    {
      name: "කසුන් රාජපක්ෂ",
      role: "Managing Director, Apex Education",
      image: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=150&auto=format&fit=crop",
      text: "අපේ ආයතනයට Google Ads සහ SEO සේවාවන් ලබා ගත්තේ ඔවුන්ගෙන්. දැන් කිසිම අමතර වියදමක් නැතුව Google සර්ච් එකෙන් දිනපතාම අලුත් ශිෂ්‍යයින් අපව සොයාගෙන එනවා."
    }
  ];
  ...
  return (
    <section className="py-20 px-6 bg-white" id="reviews">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative bg-gray-50 border border-gray-100 p-8 md:p-12 rounded-3xl shadow-xl">

          {/* Quote Icon */}
          <div className="absolute top-6 left-6 text-indigo-200 text-6xl opacity-50 font-serif select-none">“</div>
          ...
          <div className="flex flex-col items-center text-center space-y-6">
            ...
            {/* Review Text */}
            <p className="text-lg md:text-xl text-gray-700 font-medium italic max-w-2xl leading-relaxed min-h-[100px]">
          ...
          {/* Slider Navigation Buttons (z-10 සහ block එකතු කර ඇත) */}
          <div className="absolute top-1/2 -translate-y-1/2 left-2 md:-left-6 z-10">
            <button
              onClick={prevReview}
              type="button"
              className="w-10 h-10 md:w-12 md:h-12 bg-white border border-gray-100 text-gray-700 rounded-full flex items-center justify-center shadow-lg hover:bg-indigo-600 hover:text-white transition-all cursor-pointer touch-manipulation"
            >
        ...
        <div className="flex justify-center space-x-2 mt-6">
          {reviewsList.map((_, index) => (
            <button
              ...
              className={`h-2.5 rounded-full transition-all duration-300 cursor-pointer ${index === currentIndex ? 'w-8 bg-indigo-600' : 'w-2.5 bg-gray-300'}`}
            />
          ))}
        </div>
